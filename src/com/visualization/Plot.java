...
//					panel.setPreferredSize( new Dimension( width, height ) );
	...
	public void plot( List<Float> samples, final float samplesPerPixel, final Color color, boolean b )
	{			
		synchronized( image )
		{						
	...
	public void plot( List<Double> samples, final float samplesPerPixel, final Color color )
	{			
		...
		{						
			...
			Graphics2D g = image.createGraphics();
			g.setColor( color );
			double lastValue = (samples.get(0) / scalingFactor) * image.getHeight() / 3 + image.getHeight() / 2;
			for( int i = 1; i < samples.size(); i++ )
			{
				double value = (samples.get(i) / scalingFactor) * image.getHeight() / 3 + image.getHeight() / 2;
				g.drawLine( (int)((i-1) / samplesPerPixel), image.getHeight() - (int)lastValue, (int)(i / samplesPerPixel), image.getHeight() - (int)value );
				lastValue = value;
			}
			g.dispose();											
	...
	public void plot( List<Double> samples, final float samplesPerPixel, final float offset, final boolean useLastScale, final Color color )
	{
		double scalingFactor =1;
		synchronized( image )
		{
			...
			Graphics2D g = image.createGraphics();
			g.setColor( color );
			double lastValue = (samples.get(0) / scalingFactor) * image.getHeight() / 3 + image.getHeight() / 2 - offset * image.getHeight() / 3;
			for( int i = 1; i < samples.size(); i++ )
			{
				double value = (samples.get(i) / scalingFactor) * image.getHeight() / 3 + image.getHeight() / 2 - offset * image.getHeight() / 3;
				g.drawLine( (int)((i-1) / samplesPerPixel), image.getHeight() - (int)lastValue, (int)(i / samplesPerPixel), image.getHeight() - (int)value );
				lastValue = value;
			}
			g.dispose();											
